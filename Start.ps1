...
.DESCRIPTION
This curl launcher downloads one coherent source archive, validates its GitHub
HTTPS origin, extracts it into a protected temporary workspace, forwards public
installer options, propagates the installer exit code, and removes launcher
source files.
#>
...
$launcherLeafPattern = '^msft-runtime-launcher-[a-f0-9]{32}$'
...
try {
    ...
    Write-LauncherStatus -State INFO -Message 'Starting the verified PowerShell installer ...'
